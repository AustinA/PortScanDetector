...
def get_num_syn_scans(tcp_packets):
    ...
    for key in tcp_packets:
            suspects = get_suspicious_tcp_ips(tcp_packets[key])
...
def get_suspicious_tcp_ips(tcp_packets):
    
    suspicious_ips = []
    ips_to_ports = defaultdict(list)
    """
    for packet in tcp_packets:
        if packet[TCP_FLAGS] == SYN_TYPE:
            if packet[SOURCE_IP] not in ips_to_ports:
                ips_to_ports[packet[SOURCE_IP]] = [0, [], []]
                ips_to_ports[packet[SOURCE_IP]][0] += 1
                ips_to_ports[packet[SOURCE_IP]][1].append(packet[DESTINATION_PORT])
                ips_to_ports[packet[SOURCE_IP]][2] = packet
            else:
                ips_to_ports[packet[SOURCE_IP]][0] += 1
                if not packet[DESTINATION_PORT] in ips_to_ports[packet[SOURCE_IP]][1]:
                    ips_to_ports[packet[SOURCE_IP]][1].append(packet[DESTINATION_PORT])
    for key in ips_to_ports:
        if len(ips_to_ports[key][1]) > 10:
            suspicious_ips.append(ips_to_ports[key])
            """
    ...
    for ip_address in ip_addresses:
            ...
            i = 0
            while i < len(all_packets) - 1:
                packet_increment = 0
                if (all_packets[len(all_packets) - 1][TIMESTAMP] - all_packets[0][TIMESTAMP] <= SAMPLING_INVERVAL_SEC):
                    packet_increment = len(all_packets) - 1
                else:
                   current_packet_time = all_packets[i][TIMESTAMP]
                   thresh_hold = current_packet_time + SAMPLING_INVERVAL_SEC
                   elapsed_time = current_packet_time
                   while elapsed_time <= thresh_hold:
                       packet_increment += 1
                       elapsed_time = all_packets[i + packet_increment][TIMESTAMP]
                       
                   if packet_increment >= len(all_packets):
                        packet_increment = len(all_packets) - i - 1

                num_syns = 0
                for x in range(1, packet_increment + 1):
                     if all_packets[i + x][TCP_FLAGS] == SYN_TYPE:
                         suspicious_ips.append(all_packets[i + x])

                i = i + packet_increment + 1

    # Should be 1002, is only 991
